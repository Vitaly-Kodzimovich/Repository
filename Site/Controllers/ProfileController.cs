...
public class ProfileController : Controller
{
    ...
    public async Task Registration(string name, string password)
    {
        ...
        Response.Redirect("http://localhost:5252/Profile/Main");
    ...
    [HttpGet]
    public async Task Login(bool WrongPassword)
    {
        string content = "";
        
        if(WrongPassword)
        {
            content+=@"<h2>Wrong UserName or Password</h2>";
        }
    ...
    public async Task Login(string name, string password)
    {
        ...
        {
            Response.Redirect("http://localhost:5252/Profile/login?WrongPassword=true");
        }
